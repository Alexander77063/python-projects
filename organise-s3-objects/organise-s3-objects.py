...
def lambda_handler(event, context):
    ...
    bucket_name = "alex-organise-s3-objects"
    ...
    if directory_names not in get_all_s3_objects_and_folder_names:
        s3_client.put_object(Bucket=bucket_name, Key=(directory_names))
    ...
    for item in get_contents:
        ...
        if object_creation_date == directory_names and "/" not in object_name:
            s3_client.copy_object(Bucket=bucket_name, CopySource=bucket_name+"/"+object_name, Key=directory_names+object_name)
